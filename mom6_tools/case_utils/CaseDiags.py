            ...
            for line in diag_table: 
                ...
                if len(line)>0 and line[0] != '#': # not an empty line or comment line
                    ...
                    if within_file_list:
                        ...
                        self.diag_files[file_name] = file_entry
                                                
                    else: # within field list
                        field_name = line.split()[1]
                        field_line = list(line.split())
                        print(field_name, field_line)
                        field_line.remove(field_name)
                        self.diag_fields[field_name] = [entry for entry in field_line]
